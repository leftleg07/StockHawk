...
public class QuoteProvider {
    ...
    interface Path {
        ...
        String HISTORICAL_QUOTE = "historical_quotes";
    }
    ...
    @TableEndpoint(table = QuoteDatabase.HISTORICAL_QUOTE)
    public static class HistoricalQuoteData {
        @ContentUri(
                path = Path.HISTORICAL_QUOTE,
                type = "vnd.android.cursor.dir/quote"
        )
        public static final Uri CONTENT_URI = buildUri(Path.HISTORICAL_QUOTE);
        ...
        public static Uri withSymbol(String symbol) {
            return buildUri(Path.HISTORICAL_QUOTE, symbol);
        }

        @InexactContentUri(
                name = "HISTORICAL_QUOTE_ID_DATE",
                path = Path.HISTORICAL_QUOTE + "/*/*",
                type = "vnd.android.cursor.item/quote",
                whereColumn = {HistoricalQuoteColumns.SYMBOL, HistoricalQuoteColumns.DATE},
                pathSegment = {1, 2}
        )
        public static Uri withSymbolAndDate(String symbol, String date) {
            return buildUri(Path.HISTORICAL_QUOTE, symbol, date);
        }
